...
export default function AdminPage() {
  ...
  const [userTab, setUserTab] = useState<"real" | "guest" | "all">("real");
  ...
  return (
    <DashboardShell>
      ...
      <div className="space-y-8 max-w-5xl mx-auto">
        ...
        {/* AI MODEL & INTELLIGENCE ENGINE CONFIGURATION */}
        <Card className="border-purple-200/80 bg-linear-to-br from-white to-purple-50/20">
          <CardHeader>
            <CardTitle className="text-sm flex items-center gap-2">
              <Sparkles className="w-4 h-4 text-purple-600" />
              <span>AI Intelligence Engine & Gemini Model Settings</span>
            </CardTitle>
            <CardDescription>
              Configure Google Gemini models dynamically, custom API keys, and linguistic heuristic fallbacks.
            </CardDescription>
          </CardHeader>
          <CardContent>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => handleToggle("geminiModel", e.target.value)}
                    className="w-full px-3 py-2 text-xs bg-white border border-slate-200 rounded-lg text-slate-900 font-semibold focus:outline-none focus:ring-1 focus:ring-purple-500 cursor-pointer shadow-2xs"
                  >
                    <option value="gemini-2.5-flash">
                      ⚡ Gemini 2.5 Flash (Recommended - Fastest & Latest)
                    </option>
                    <option value="gemini-2.0-flash">
                      🚀 Gemini 2.0 Flash (Next-Gen High Speed)
                    </option>
                    <option value="gemini-1.5-flash">
                      ⚙️ Gemini 1.5 Flash (High Throughput & Speed)
                    </option>
                  ...
                  <span className="text-[11px] text-slate-400 mt-1 block">
                    All document reviews and rewrites will automatically route through this model.
                  </span>
                </div>
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => handleToggle("aiProvider", e.target.value as any)}
                    className="w-full px-3 py-2 text-xs bg-white border border-slate-200 rounded-lg text-slate-900 font-semibold focus:outline-none focus:ring-1 focus:ring-purple-500 cursor-pointer shadow-2xs"
                  >
                    <option value="gemini">Google Gemini API (Cloud Intelligence)</option>
                    <option value="heuristics">Linguistic Heuristics (Local Fallback Mode)</option>
                  </select>
                  <span className="text-[11px] text-slate-400 mt-1 block">
                    Heuristics mode works 100% offline without external API keys.
                  </span>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    placeholder="AIzaSy..."
                    className="flex-1 px-3 py-2 text-xs bg-white border border-slate-200 rounded-lg text-slate-900 font-mono shadow-2xs"
                  />
                  <Button
                    type="button"
                    size="sm"
                    onClick={async () => {
                      setIsSaving(true);
                      await updateAdminConfigAction({ geminiApiKey: config.geminiApiKey });
                      setIsSaving(false);
                ...
                <span className="text-[11px] text-slate-400 mt-1 block">
                  Leaves empty to use system environment variable (<code className="bg-slate-100 px-1 py-0.5 rounded text-slate-800">GEMINI_API_KEY</code>).
                </span>
              </div>
            </div>
          </CardContent>
        ...
        {/* 3. RATE LIMITING & DYNAMIC WORD LIMITS */}
        <Card>
          <CardHeader>
            <CardTitle className="text-sm flex items-center gap-2">
              <Sliders className="w-4 h-4 text-indigo-600" />
              <span>Rate Limiting & Dynamic Word Limits</span>
            </CardTitle>
            <CardDescription>
              Configure document size, word count ceilings, and daily allowances enforced server-side.
            </CardDescription>
          </CardHeader>
          <CardContent>
            <form onSubmit={handleSaveRateLimits} className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    max="50000"
                    step="500"
                    value={config.rateLimits.maxWordsPerDocument || 10000}
                    onChange={(e) =>
                      setConfig({
                        ...config,
                        rateLimits: {
                          ...config.rateLimits,
                          maxWordsPerDocument: parseInt(e.target.value) || 10000,
                        },
                      })
                    }
                    className="w-full px-3 py-1.5 text-xs bg-slate-50 border border-slate-200 rounded-lg text-slate-900 font-medium"
                ...
                <div>
                  ...
                  <input
                    ...
                    max="3000"
                    step="50"
                    value={config.rateLimits.maxWordsPerRewrite || 800}
                    onChange={(e) =>
                      setConfig({
                        ...config,
                        rateLimits: {
                          ...config.rateLimits,
                          maxWordsPerRewrite: parseInt(e.target.value) || 800,
                        },
                      })
                    }
                    className="w-full px-3 py-1.5 text-xs bg-slate-50 border border-slate-200 rounded-lg text-slate-900 font-medium"
              ...
              <div className="pt-3 flex justify-end">
                <Button type="submit" size="sm" isLoading={isSaving} className="cursor-pointer">
                  <Save className="w-3.5 h-3.5 mr-1.5" />
                  <span>Save Rate Limiting & Word Rules</span>
                </Button>
              </div>
            </form>
          </CardContent>
        ...
        <Card>
          <CardHeader>
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div>
                <CardTitle className="text-sm flex items-center gap-2">
                  ...
                  <span>User Accounts Directory & Roles</span>
                </CardTitle>
                <CardDescription>
                  View real registered accounts, authentication methods, and manage temporary guest sessions.
                </CardDescription>
              </div>

              {/* Stats badges */}
              <div className="flex items-center gap-2">
                <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-emerald-50 text-emerald-700 font-bold text-xs border border-emerald-200/80">
                  <UserCheck className="w-3.5 h-3.5 text-emerald-600" />
                  <span>{users.filter((u) => !u.isGuest).length} Real Registered</span>
                </span>
                <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-slate-100 text-slate-600 font-bold text-xs border border-slate-200">
                  <span>{users.filter((u) => u.isGuest).length} Guests</span>
                </span>
              </div>
            </div>
            ...
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 pt-3 border-t border-slate-100 mt-2">
              <div className="inline-flex p-1 bg-slate-100 rounded-lg border border-slate-200/80">
                ...
                >
                  Real Users ({users.filter((u) => !u.isGuest).length})
                </button>
                <button
                  type="button"
                ...
                >
                  Temporary Guests ({users.filter((u) => u.isGuest).length})
                </button>
                <button
                  type="button"
          ...
          <CardContent>
            {(() => {
              const realList = users.filter((u) => !u.isGuest);
              const guestList = users.filter((u) => u.isGuest);
              const baseList =
                userTab === "real"
                  ? realList
                  : userTab === "guest"
              ...
              if (filtered.length === 0) {
                return (
                  <div className="py-12 text-center text-slate-400 text-xs">
                    {userTab === "real"
                      ? "No real registered users found matching your search."
                      : userTab === "guest"
                      ? "No temporary guest users found."
                    ...
                    <tbody className="divide-y divide-slate-100 text-slate-700 font-medium">
                      {filtered.map((u) => (
                        <tr key={u.id} className="hover:bg-slate-50/50 transition-colors">
                          ...
                          <td className="py-3 px-3">
                            <span className="font-mono text-slate-600 font-medium">{u.email}</span>
                          </td>
                          <td className="py-3 px-3">
                            {!u.isGuest ? (
                          ...
                          <td className="py-3 px-3">
                            <span className="px-2 py-0.5 rounded-md text-[10px] font-medium bg-slate-100 text-slate-700 border border-slate-200/80">
